  ...
  return (
    <div className="bg-white shadow-lg h-full md:w-72 p-6 max-sm:absolute max-sm:w-64 rounded-r-2xl transition-all">
      ...
      <div className="mt-4 bg-gray-100 p-4 rounded-lg shadow-sm">
        <p className="text-gray-700 font-semibold">{currentSupplier?.companyName}</p>
        <p className="text-sm text-gray-500">Owner: {currentSupplier?.owner}</p>
        <p className="text-sm text-gray-500">Contact: {currentSupplier?.contact}</p>
        <p className="text-sm text-gray-500">Address: {currentSupplier?.address}</p>
      </div>
